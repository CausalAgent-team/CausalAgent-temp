...
或学术 Web 证据工具；知识库证据工具是否必须调用由本次运行的受控提示决定。工具
调用由程序记录；不要声称没有发生的调用、结果、证据、图边或算法结论。
...
MANDATORY_RAG_INSTRUCTION = (
    "本次运行要求始终检索知识库：必须至少调用一次 rag_evidence_search，"
    "并等待它返回真实状态后再提交最终结构化决策。即使返回无相关证据或暂不可用，"
    "也必须保留这次真实检索状态，不能假装已经获得证据。"
)
